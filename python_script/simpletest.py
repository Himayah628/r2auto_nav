...
class AutoNav(Node):
    ...
    def irsensor(self):
         while 1:
             if(GPIO.input(Left_IR)==False and GPIO.input(Right_IR)==False):  #stop
                 self.get_logger().info('none')
             elif(GPIO.input(Left_IR)==False and GPIO.input(Right_IR)==True): #turn left 
                 self.get_logger().info('left one detects')
 
             elif(GPIO.input(Left_IR)==True and GPIO.input(Right_IR)==False): #turn right
                 self.get_logger().info('right one detects')
 
             else:  #forward
                 self.get_logger().info('both detecty')
    ...
    def moveforward(self, detect_angles, detect_distanct, reverse=False):
        front_angles = range(detect_angles-10,detect_angles+10,1)
        twist = Twist()
        twist.linear.x = 0.1
        if(reverse):
            twist.linear.x = -0.1
    ...
    def returning_from_table(self, table_no):
        ...
        if(table_no == 4 or table_no == 3):
            self.moveforward(180, 0.25, reverse=True)
            self.rotatebot(anticlockwise_angle)
