...
class AutoNav(Node):
    ...
    def moveforward(self, detect_angles, detect_distanct,reverse=False, angle = 10):
        front_angles = range(detect_angles-angle,detect_angles+angle,1)
        twist = Twist()
        twist.linear.x = 0.1
        if(reverse):
            twist.linear.x = -0.1
        ...
        while rclpy.ok():
            if self.laser_range.size != 0:
                ...
                if(len(lri[0])>0):
                    ...
                    break
            rclpy.spin_once(self)
    def orientationcheck(self,angle):
        while rclpy.ok():
            if self.laser_range.size != 0:
                detect_angle=range(angle-10,angle+10,1)
                distance_atangle=self.laser_range[detect_angle].nonzero()
                lr2i = np.nanargmin(distance_atangle)
                while lr2i!=angle:
                    twist = Twist()
                    if lr2i<angle:
                        twist.linear.x = 0.0
    ...
    def travelling_to_table(self, table_no):
        ...
        if(table_no ==5):
            ...
            self.moveforward(0, 0.30)
            self.rotatebot(anticlockwise_angle)
            self.orientationcheck(180) #use lidar to confirm
            self.moveforward(0, 0.30) #reach table 5
        
            
        else:
            self.rotatebot(anticlockwise_angle)
            self.moveforward(0, 0.30) #reach table 1
            if(table_no != 1):
                self.rotatebot(clockwise_angle)
                self.orientation(180) 
                if(table_no == 6):
                    self.moveforward(0, 0.6)
                    self.rotatebot(anticlockwise_angle)
                    self.orientationcheck(270)# use lidar to confirm
                    #stop after passing line
                    self.moveforward(0, 1.33)
                    ...
                    self.moveforward(90,table6max,0)
                    self.rotatebot(anticlockwise_angle)
                    self.orientationcheck(180)
                    self.moveforward(0,0.3) #reach table 6
                    ...
                    if(table_no ==2):
                        ...
                        if(table_no != 3):
                            self.rotatebot(anticlockwise_angle) 
                            self.orientationcheck(90)
                            self.moveforward(270, 0.30) #reach table 4
    ...
    def returning_from_table(self, table_no):
        while True:
            if self.proximity():
                ...
                break
     if (table_no == 6):
         self.moveforward(180, 0.6, reverse = True)
         self.rotatebot(clockwise_angle)
         self.orientation(270)
         self.moveforward(180, 0.30, reverse = True)
         self.rotatebot(clockwise_angle)
        if(table_no == 4 or table_no == 6):
            self.moveforward(180, 0.30, reverse=True)
            self.rotatebot(anticlockwise_angle)
            self.irsensor()
        elif(table_no == 3):
            ...
            self.rotatebot(clockwise_angle)
            self.moveforward(90, 0.30, reverse=True)
            self.rotatebot(anticlockwise_angle)
            self.irsensor()
        elif(table_no == 1):
            self.irsensor()
